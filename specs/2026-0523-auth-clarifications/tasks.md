...
```
- [X] Carpeta src/domain/ existe con subcarpetas:
      - entities/
      - value_objects/
      - interfaces/
      - exceptions.py
- [X] Carpeta src/application/ existe con subcarpeta:
      - auth/
- [X] Carpeta src/infrastructure/ existe con subcarpetas:
      - persistence/
      - config.py
- [X] Carpeta src/presentation/ existe con subcarpetas:
      - routers/
      - schemas/
      - dependencies/
      - main.py
- [X] Archivo .env existe con variables:
...
- [X] Archivo requirements.txt contiene:
      fastapi>=0.104.0
      sqlalchemy>=2.0.0
      ...
      uvicorn>=0.24.0
- [X] Todos los __init__.py creados en carpetas de módulos
```
...
**Criterios de Aceptación:**
```
- [X] Clase Email existe en src/domain/value_objects/email.py
- [X] Constructor recibe un string y lanza ValueError si es inválido
- [X] Validación usa email-validator.validate_email()
- [X] Email es inmutable: no hay setters
- [X] __eq__ compara correctamente dos instancias Email
- [X] __hash__ permite usar Email como clave en dict/set
- [X] __str__ retorna la dirección de email
- [X] Ejemplo de uso:
      email1 = Email("user@example.com")
      email2 = Email("user@example.com")
      assert email1 == email2
      assert hash(email1) == hash(email2)
- [X] Email("invalid") lanza ValueError
- [X] Email("") lanza ValueError
- [X] Pattern comentado en código: # Pattern: Value Object
```
...
**Criterios de Aceptación:**
```
- [X] Clase Password existe en src/domain/value_objects/password.py
- [X] Interfaz HashingStrategy abstracta existe:
      - método hash(plaintext: str) -> str
      - método verify(plaintext: str, hash: str) -> bool
- [X] Clase BcryptStrategy implementa HashingStrategy
- [X] Clase BcryptStrategy.hash():
      - Usa bcrypt.gensalt(rounds=12)
      - Retorna hash como string UTF-8
- [X] Clase BcryptStrategy.verify():
...
- [X] Método Password.verify(plaintext: str) -> bool
- [X] Password("pass") lanza ValueError (menos de 8 caracteres)
- [X] Password("password") lanza ValueError (sin mayúscula)
- [X] Password("Password1") NO lanza (válido)
- [X] Ejemplo:
      pwd = Password("TestPass123")
      assert pwd.verify("TestPass123") == True
      assert pwd.verify("WrongPass") == False
- [X] Pattern comentado: # Pattern: Value Object + Strategy
```
...
**Criterios de Aceptación:**
```
- [X] Clase User existe en src/domain/entities/user.py
- [X] Atributos:
      - id: UUID (inmutable)
      - email: Email VO
      - password: Password VO
      - created_at: datetime (inmutable)
- [X] Constructor privado (o protegido): User.__init__() o User._init()
- [X] Static Factory Method User.create(email: str, password: str) -> User:
...
- [X] User.create("invalid@", "Pass123") lanza ValueError
- [X] User.create("john@example.com", "weak") lanza ValueError
- [X] Pattern comentado: # Pattern: Entity + Factory Method
```
...
**Criterios de Aceptación:**
```
- [X] Archivo src/domain/interfaces/i_user_repository.py existe
- [X] Clase IUserRepository es ABC (ABC de abc module)
- [X] Método abstracto:
...
- [X] Pattern comentado: # Pattern: Repository (Domain Interface)
```
...
**Criterios de Aceptación:**
```
- [X] Archivo src/domain/exceptions.py existe
- [X] Clase UserAlreadyExistsError(Exception):
      - mensaje: "User with this email already exists"
- [X] Clase InvalidCredentialsError(Exception):
      - mensaje: "Invalid email or password"
- [X] Clase UserNotFoundError(Exception):
      - mensaje: "User not found"
- [X] Cada excepción hereda de Exception
- [X] Ejemplo:
...
**Criterios de Aceptación:**
```
- [X] Archivo src/infrastructure/config.py existe
- [X] Clase Settings hereda BaseSettings (pydantic_settings)
- [X] Atributos en Settings:
      - database_url: str (desde DATABASE_URL env)
      - jwt_secret_key: str (desde JWT_SECRET_KEY env)
      - jwt_algorithm: str = "HS256"
      - jwt_expiration_hours: int = 24
- [X] Archivo src/infrastructure/persistence/database.py existe
- [X] Crear SQLAlchemy engine:
      engine = create_engine(
          settings.database_url,
          poolclass=NullPool,  # Development
          echo=False
      )
- [X] Crear SessionLocal:
      SessionLocal = sessionmaker(
          autocommit=False,
          autoflush=False,
          bind=engine
      )
- [X] Crear declarative_base:
...
- [X] settings singleton importable:
...
**Criterios de Aceptación:**
```
- [X] Archivo src/infrastructure/persistence/models.py existe
- [X] Clase UserModel hereda Base (declarative_base)
...
**Criterios de Aceptación:**
```
- [X] Archivo src/infrastructure/persistence/user_repository.py existe
- [X] Clase UserRepository(IUserRepository):
- [X] Constructor:
      __init__(self, session: Session):
          self.session = session
- [X] Método create(user: User) -> User:
...
- [X] Método find_by_email(email: Email) -> Optional[User]:
      - Query: SELECT * FROM users WHERE email = ?
      - Si existe: convertir UserModel → User Entity
      - Si no existe: retorna None
- [X] Método email_exists(email: Email) -> bool:
      - Query: SELECT COUNT(*) FROM users WHERE email = ?
      - Retorna count > 0
- [X] Helper privado _model_to_entity(model: UserModel) -> User:
...
**Criterios de Aceptación:**
```
- [X] Archivo src/application/auth/jwt_service.py existe
- [X] Clase JWTService:
- [X] Constructor:
      __init__(self, settings: Settings):
          self.settings = settings
- [X] Método generate(user_id: str, email: str, role: str = "user") -> str:
...
- [X] Método verify(token: str) -> dict:
...
- [X] Método extract_user_id(payload: dict) -> str:
      - Retorna payload["sub"]
- [X] Ejemplo:
...
**Criterios de Aceptación:**
```
- [X] Archivo src/application/auth/auth_service.py existe
- [X] Clase AuthService:
- [X] Constructor:
      __init__(
          self,
...
- [X] Método register(email: str, password: str) -> dict:
...
- [X] Método login(email: str, password: str) -> dict:
...
**Criterios de Aceptación:**
```
- [X] Archivo src/presentation/schemas/auth_schemas.py existe
- [X] Clase RegisterRequest(BaseModel):
      - email: str
      - password: str
- [X] Clase RegisterResponse(BaseModel):
      - user_id: str
- [X] Clase LoginRequest(BaseModel):
      - email: str
      - password: str
- [X] Clase LoginResponse(BaseModel):
      - access_token: str
      - token_type: str = "bearer"
- [X] Clase MeResponse(BaseModel):
      - user_id: str
- [X] Validación adicional (opcional):
...
**Criterios de Aceptación:**
```
- [X] Archivo src/presentation/dependencies/auth.py existe
- [X] Función get_current_user(
          request: Request,
...
**Criterios de Aceptación:**
```
- [X] Archivo src/presentation/routers/auth.py existe
...
**Criterios de Aceptación:**
```
- [X] Archivo src/presentation/main.py existe
- [X] Instancia FastAPI:
      app = FastAPI(
...
- [X] Registro de routers:
      app.include_router(auth_router, prefix="/api/v1", tags=["auth"])
- [X] Inyección de dependencias (FastAPI Lifespan o startup event):
...
- [X] Endpoint GET /health (sin autenticación):
...
**Criterios de Aceptación:**
```
- [X] Archivo src/main.py o app.py existe en raíz proyecto
- [X] Contenido:
      import uvicorn
...
**Criterios de Aceptación:**
```
- [X] Archivo README.md existe en raíz
- [X] Secciones:
